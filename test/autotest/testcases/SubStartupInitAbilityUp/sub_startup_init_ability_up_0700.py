...
class SubStartupInitAbilityUp0700(TestCase):
    ...
    def test_step1(self):
        ...
        self.driver.pull_file(devpath, "testFile/SubStartupInitAbilityUp/hilogd.cfg")
        Step("修改本地系统文件..........................")
        lines1 = []
        path2 = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))),
                             "testFile\SubStartupInitAbilityUp")
        localpath = os.path.join(path2, "hilogd.cfg")
        with open(localPath, "r") as y:
            for line in y:
                lines1.append(line)
            y.close()
        line = lines1.index('            "socket" : [{\n') - 1
        res1 = line
        Step(f"目标索引：{res1}")
        ...
        list_item = '            "file" : ["/dev/test/kmsg rd 0640 root root"],\n'
        ...
        Step(result)
        self.driver.Assert.contains(result, "HiLogAdapter_init: Can't connect to server. Errno: 2")
