...
class SUB_STARTUP_INIT_SERIAL_0300(TestCase):
    ...
    def process(self):
        global deviceinfo_list, osFullName
        ...
        version_num = []
        version_lst = ["majorVersion", "seniorVersion", "featureVersion", "buildVersion"]
        for deviceinfo in deviceinfo_list:
            if deviceinfo.split()[0].startswith("osFullName"):
                osFullName = deviceinfo.split()[1]
            for version in version_lst:
                if deviceinfo.split()[0].startswith(version):
                    version_num.append(deviceinfo.split()[1])
        self.driver.Assert.contains(osFullName, '.'.join(version_num))
