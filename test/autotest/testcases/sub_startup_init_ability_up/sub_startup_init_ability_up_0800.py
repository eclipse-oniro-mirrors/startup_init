...
class SubStartupInitAbilityUp0800(TestCase):
    ...
    def test_step1(self):
        ...
        self.driver.pull_file(devpath, "testFile/sub_startup_init_ability_up/wifi_standard.cfg")
        Step("修改本地系统文件..........................")
        lines1 = []
        path2 = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))),
                             "testFile\sub_startup_init_ability_up")
        local_path = os.path.join(path2, "wifi_standard.cfg")
        with open(local_path, "r") as y:
            for line in y:
                lines1.append(line)
            y.close()
        line = lines1.index('            "uid" : "wifi",\n')
        res1 = line
        Step(f"目标索引：{res1}")
