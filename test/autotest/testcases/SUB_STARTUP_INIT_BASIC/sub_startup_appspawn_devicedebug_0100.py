...
SubStartupAppspawnDevicedebug0100(TestCase):
    ...
    def process(self):
        ...
        hap_path = Common.sourcepath('release.hap', "sub_startup_appspawn_devicedebug")
        ...
        self.driver.Assert.equal(pid1, pid2)

        Step("匹配日志")
        device_logger.stop_log()
        time.sleep(10)
        assert device_logger.check_log("devicedebug cmd kill start signal[9], pid[%d]" % pid1)
        assert device_logger.check_log("appspawn devicedebug process is not debuggable, pid=%d" % pid1)
