...
class SubStartupAppspawnAppclone0100(TestCase):
    ...
    def test_step1(self):
        ...
        self.driver.shell("touch ../data/app/el2/100/database/+clone-1+com.ohos.mytest/123.txt")
        Step("步骤5：分身应用沙盒路径是否存在123.txt")
        cpid = self.driver.System.get_pid("com.ohos.mytest1")
        result1 = self.driver.shell("ls ../proc/%d/root/data/storage/el2/database" % cpid)
        Step("步骤6：预期结果校验")
        self.driver.Assert.contains(result1, "123.txt")
        Step("步骤7：主应用沙盒路径是否存在123.txt")
        pid = self.driver.System.get_pid("com.ohos.mytest")
        result2 = self.driver.shell("ls ../proc/%d/root/data/storage/el2/database" % pid)
        Step("步骤6：预期结果校验")
        if ("123.txt" in result2):
