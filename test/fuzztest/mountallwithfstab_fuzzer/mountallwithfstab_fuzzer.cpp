 ...
 */
#include "mountallwithfstab_fuzzer.h"
#include <iostream>
#include "fs_manager/fs_manager.h"

namespace OHOS {
    bool FuzzMountAllWithFstab(const uint8_t* data, size_t size)
    {
        bool result = false;
        FILE *pFile = nullptr;
        ...
        if (!MountAllWithFstab(fstab, false)) {
            result = true;
        }
        return result;
    }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::FuzzMountAllWithFstab(data, size);
    return 0;
}
