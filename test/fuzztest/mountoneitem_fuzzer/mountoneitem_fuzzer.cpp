...
#include "mountoneitem_fuzzer.h"
#include <iostream>
#include <string>
#include "fs_manager/fs_manager.h"

namespace OHOS {
    bool FuzzMountOneItem(const uint8_t* data, size_t size)
    {
        bool result = false;
        FILE *pFile = nullptr;
        ...
        FstabItem *item = FindFstabItemForPath(*fstab, str.c_str());
        if (!MountOneItem(item)) {
            result = true;
        };
        return result;
    }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::FuzzMountOneItem(data, size);
    return 0;
}
