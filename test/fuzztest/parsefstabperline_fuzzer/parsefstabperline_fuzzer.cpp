...
#include "parsefstabperline_fuzzer.h"
#include <iostream>
#include <string>
#include "fs_manager/fs_manager.h"

namespace OHOS {
    bool FuzzParseFstabPerLine(const uint8_t* data, size_t size)
    {
        bool result = false;
        FILE *pFile = nullptr;
        ...
        if (ParseFstabPerLine(nullptr, fstab, false, str.c_str())) {
            result = true;
        };
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::FuzzParseFstabPerLine(data, size);
    return 0;
}
