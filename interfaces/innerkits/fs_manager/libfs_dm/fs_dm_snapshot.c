...
{
    BEGET_LOGI("ParseStatusText start");
    ...
    BEGET_LOGE("could not parse snapshot processInfo: wrong format");
    return false;
}

bool GetDmMergeProcess(const char *name, StatusInfo *processInfo)
{
    BEGET_LOGI("GetDmMergeProcess start, name %s", name);
    ...
    do {
        ...
        if (strcmp(spec->target_type, "snapshot-merge") != 0 || !ParseStatusText(data, processInfo)) {
            BEGET_LOGE("get merge process fail");
            free(data);
            break;
        }
        free(data);
    ...
    BEGET_LOGI("GetDmMergeProcess end. ret %d", ret);
    close(fd);
    free(buffer);
    return ret;
}
